...
namespace ts::client::cli {
    ...
    void Presentation::Print( const protocol::SessionEvent& event ) {
        std::visit(
            [this]( const auto& value ) {
                ...
                } else if constexpr ( std::is_same_v<EventType, protocol::PokeEvent> ) {
                    PrintPoke( value );
                }
            },
            event );
    ...
    void Presentation::PrintPoke( const protocol::PokeEvent& event ) {
        m_Notify( ts::audio::NotificationType::Reminder );

        if ( m_UseColor ) {
            // Red + italic: distinct from every existing message color
            // (Magenta/Green/Yellow chat, plain Red errors, Cyan+italic
            // info/presence) while still reading as urgent.
            log::ApplyTerminalStyle( m_Output,
                                     log::TerminalStream::StandardOutput,
                                     { .color = log::TerminalColor::Red, .italic = true } );
        }

        m_Output << "[poke] ";
        WriteSafe( event.invokerName.empty() ? "client " + std::to_string( event.invokerId ) : event.invokerName );
        m_Output << ": ";
        WriteSafe( event.message );

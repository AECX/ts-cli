...
namespace ts::net {
    ...
    std::optional<SrvRecord> ResolveSrvRecord( std::string_view service, std::string_view proto, std::string_view domain ) {
        ...
        for ( int index = 0; index < count; ++index ) {
            ...
            SrvRecord candidate;
            // ns_get16() reads exactly two bytes, so the result always fits
            // in 16 bits; the conversion is lossless despite the u_int return type.
            candidate.priority = static_cast<std::uint16_t>( ns_get16( data ) );
            candidate.weight = static_cast<std::uint16_t>( ns_get16( data + 2 ) );
            candidate.port = static_cast<std::uint16_t>( ns_get16( data + 4 ) );

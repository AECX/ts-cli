...
`AudioEngine` owns a worker thread plus the platform backend's capture/playback callbacks (PipeWire on Linux, WASAPI on Windows — see [Audio and voice](audio.md)).

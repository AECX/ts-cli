...
namespace ts::protocol {
    ...
    void Session::ProcessCommand( const Command& command ) {
        ...
        if ( command.Name() == "notifyclientpoke" ) {
            const ClientPoke poke = ClientPoke::Parse( command );
            const ClientPokeEntry& entry = poke.Entry();

            m_Events.emplace_back( PokeEvent { .invokerId = entry.invokerId,
                                               .invokerName = entry.invokerName,
                                               .invokerUniqueId = entry.invokerUniqueId,
